...
<div {{ $attributes->class(Arr::toCssClasses($containerClass)) }}>
    <div class="flex items-center gap-x-1" data-slot="alert-heading">
        ...
        <div class="flex-1 text-start text-black dark:text-white font-medium">
            {{ $heading ?? $slot }}
        </div>
    </div>

    @if (isset($content) && !$content->isEmpty())
        <div class="text-start text-sm text-neutral-700 dark:text-neutral-400" data-slot='alert-content'>
            {{ $content }}
        </div>
    @endif
